<template>
    <header class="flex justify-between font-extrabold p-4">
        ...
        <div class="flex items-center justify-center ">
            ...
            <label>
                <svg class="h-4 w-4 md:h-6 md:w-6 fill-dark-notification dark:fill-White" fill="#000000" width="800px"
                    height="800px" viewBox="-5.5 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                    <title>light</title>
                    <path
            ...
            <input type="checkbox" @click="toggleDark()" :checked="isDark"
                class="cursor-pointer flex items-center appearance-none w-9 focus:outline-none checked:bg-Blue h-5 bg-Grayish-blue rounded-full before:inline-block before:rounded-full before:bg-Very-light-grayish-blue before:h-4 before:w-4 checked:before:translate-x-full shadow-inner transition-all duration-300 before:ml-0.5" />
            <!-- dark icon -->
            <label>
                <svg class="h-4 w-4 md:h-6 md:w-6 fill-dark-notification dark:fill-White" fill="#000000" width="800px"
        ...
        <button @click="$emit('mark-all-as-read')" class="text-Dark-grayish-blue font-medium hover:text-Blue">Mark all as
            read</button>
    </header>
</template>
...
const props = defineProps<{ unreadMessages: number }>();
</script>
